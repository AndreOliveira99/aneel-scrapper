/* Esse código realiza requisições no portal cnpj rocks para obter dados a partir do cnpj e razão social
   ...
   function getCompanyDataOutput(htmlString, companyDataInput, url) {
       let companyDataOutput = companyDataInput
       ...
       companyDataOutput.Url = url
       return companyDataOutput
   }
   
   makeRequest = async (cnpj, companyName, index) => {
       let url = `https://cnpjs.rocks/cnpj/${cnpj}/${companyName}.html`
       console.log(url)
       var config = {
           method: 'get',
           url: url,
       ...
       let companyDataOutput = await axios(config)
           .then(function (response) {
               ...
               return getCompanyDataOutput(response.data, dataInput[index], url)
           })
   ...
   async function getFinalList(cnpjArray, companyNameArray) {
       let companyList = []
       for (i = 0; i < cnpjArray.length; i++) { // Trocar o 100 para cnpjArray.length
           try {
               if (i % 5 == 0) { // É possível mudar a quantidade de requisições em paralelo mudando o número 1 para o número de operações paralelas desejadas. 
                   await new Promise(resolve => setTimeout(resolve, 500)) // Spleep artifical pra esperar 5 segundos entre as requisições.
                   companyList[i] = makeRequest(cnpjArray[i], companyNameArray[i], i)
               }
               else {
                   companyList[i] = makeRequest(cnpjArray[i], companyNameArray[i], i)
               }
       ...
       return Promise.all(companyList).then(response => {
           return response
       })
   }
   ...
   getFinalList(cnpjArray, companyNameArray)
       .then(newJson => {
           var filename = path.join(__dirname, '../json_files/teste-sincronismo.json');
